...
const NarrativePane: React.FC<NarrativePaneProps> = ({ isOpen, title, content, onClose }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative w-full max-w-4xl max-h-[90vh] bg-card rounded-xl shadow-xl transform transition-all duration-300 flex flex-col",
          isOpen ? "scale-100 opacity-100" : "scale-95 opacity-0"
        )}
        onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside modal
      >
        <div className="p-6 pb-4 flex justify-between items-center border-b border-border flex-shrink-0">
          <h2 className="text-xl font-bold text-foreground">{title}</h2>
          <Button variant="ghost" size="icon" onClick={onClose} className="rounded-full h-8 w-8">
            <X className="h-4 w-4" />
          </Button>
        </div>
        <div className="px-6 pt-4 pb-6 min-w-0 overflow-y-auto prose max-w-none 
